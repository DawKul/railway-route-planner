        ...
        map.on('pm:create', (e) => {
            ...
            if (layer instanceof L.CircleMarker) {
                ...
                const createStopPopupContent = (props, stop) => {
                    // Get all other stops for the connection dropdown
                    const otherStops = stopsRef.current.filter(s => s !== stop && s.properties.name !== props.name);
                    const currentConnections = stop.connections?.map(conn => 
                        `<div class="connection-item" style="display: flex; justify-content: space-between; align-items: center; padding: 4px 0;">
                            <span>${conn.connectedTo.properties.name} (${conn.isOutgoing ? '→ Do' : '← Z'})</span>
                            <button class="remove-connection" data-stop="${conn.connectedTo.properties.name}" 
                            style="color: red; border: none; background: none; cursor: pointer; padding: 0 4px;">×</button>
                        </div>`
                    ) || [];
                ...
                marker.on('popupopen', () => {
                    setTimeout(() => {
                        ...
                        saveBtn.onclick = () => {
                            ...
                            // Refresh all popups to update any references to this stop
                            refreshAllStopPopups();
