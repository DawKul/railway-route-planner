...
app.post('/routes', async (req, res) => {
    ...
    try {
        ...
        // Debug: Sprawdzanie struktury każdego przystanku
        if (stops && stops.length > 0) {
            stops.forEach((stop, index) => {
                console.log(`Przystanek ${index}:`, {
                    name: stop.properties?.name,
                    stopTime: stop.properties?.stopTime,
                    passengersIn: stop.properties?.passengersIn,
                    passengersOut: stop.properties?.passengersOut,
                    isTerminal: stop.properties?.isTerminal,
                    rawProperties: stop.properties
                });
            });
        }
            ...
            if (stops && Array.isArray(stops) && stops.length > 0) {
                for (let i = 0; i < stops.length; i++) {
                    ...
                    console.log('Dodawanie przystanku:', {
                        ...
                        passengersIn: s.properties?.passengersIn,
                        passengersOut: s.properties?.passengersOut,
                        isTerminal: s.properties?.isTerminal,
                        rawProperties: s.properties
                    });
                    
                    // Debug: wyświetl wartości przed zapisem
                    const insertValues = [
                        route.route_id,
                        i + 1,
                        s.properties.name,
                        ...
                        parseInt(s.properties?.passengersIn) || 0,
                        parseInt(s.properties?.passengersOut) || 0
                    ];
                    
                    console.log('Wartości do zapisu:', {
                        route_id: insertValues[0],
                        seq_no: insertValues[1],
                        name: insertValues[2],
                        lon: insertValues[3],
                        lat: insertValues[4],
                        stop_time: insertValues[5],
                        is_terminal: insertValues[6],
                        passengers_in: insertValues[7],
                        passengers_out: insertValues[8]
                    });
